...
class Show {
   ...
   public Show(String var1, String var2, String var3, String var4, String var5, String var6, String var7, int var8,
         String var9, String var10, String var11) {
      this.show_id = var1;
      this.tipo = var2;
      this.titulo = var3;
      this.diretor = var4;
   ...
   public Show clone() {
      return new Show(this.show_id, this.tipo, this.titulo, this.diretor, this.elenco, this.pais, this.dataAdicionado,
            this.anoLancamento, this.classificacao, this.duracao, this.categorias);
   }

   public void imprimir() {
      System.out.print("=> " + this.show_id + " ## " + this.titulo + " ## " + this.tipo + " ## " + this.diretor
            + " ## [" + this.elenco + "] ## " + this.pais + " ## " + this.dataAdicionado + " ## " + this.anoLancamento
            + " ## " + this.classificacao + " ## " + this.duracao + " ## [" + this.categorias + "] ##\n");
   }
...
public class q11 {
   ...
   public static void main(String[] args) throws Exception {
      ...
      FileWriter log = new FileWriter("matricula_countingsort.txt");
      log.write("793406" + "\t" + tempoExecucao + "\t" + comparacoes);
      log.close();
   }

   public static int countingComparacoes = 0;

   public static void countingSort(Show[] array, int n) {
      // Encontrar o maior ano de lançamento usando getter
      int maxAno = 0;
      for (int i = 0; i < n; i++) {
         if (array[i].getAnoLancamento() > maxAno) {
            maxAno = array[i].getAnoLancamento();
         }
      }

      // Criar vetor de contagem
      int[] count = new int[maxAno + 1];
      for (int i = 0; i < n; i++) {
         count[array[i].getAnoLancamento()]++;
      }
      ...
      for (int i = n - 1; i >= 0; i--) {
         int ano = array[i].getAnoLancamento();
         ordenado[--count[ano]] = array[i];
      }

      // Ordenar cada grupo de mesmo ano por título usando insertion sort
      int i = 0;
      while (i < n) {
         int anoAtual = ordenado[i].getAnoLancamento();
         int j = i + 1;
         while (j < n && ordenado[j].getAnoLancamento() == anoAtual) {
            countingComparacoes++;
            j++;
         }
         insertionSortPorTitulo(ordenado, i, j - 1);
         i = j;
      }
   ...
   public static void insertionSortPorTitulo(Show[] array, int inicio, int fim) {
      for (int i = inicio + 1; i <= fim; i++) {
         Show temp = array[i];
         int j = i - 1;
         while (j >= inicio && array[j].getTitulo().compareTo(temp.getTitulo()) > 0) {
            countingComparacoes++;
            array[j + 1] = array[j];
            j--;
         }
