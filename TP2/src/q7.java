...
class Show {
 ...
 public class q7{
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Ordena os shows encontrados com algoritimo de inserção
        for(int i = 1; i < totalInseridos; i++) {
            Show tmp = vetorInseridos[i];
            int j = i - 1;
            
            while (j >= 0 && (
                   vetorInseridos[j].getTipo().compareTo(tmp.getTipo()) > 0 ||
                  (vetorInseridos[j].getTipo().compareTo(tmp.getTipo()) == 0 &&
                   vetorInseridos[j].getTitulo().compareTo(tmp.getTitulo()) > 0))) {
                
                vetorInseridos[j + 1] = vetorInseridos[j];
                j--;
            }
            vetorInseridos[j + 1] = tmp;
        }
